...
function buildAssociations(model, models) {
  ...
  if (model) {
    _.forEach(model.associations, (association) => {
      if (association.model && models[association.model] && models[association.model].primaryKey) {
      ...
      } else if (association.collection && association.via && models[association.collection] && models[association.collection].primaryKey) {
        ...
        builtAssociations[association.alias] = {
          ...
          resolve: (parentObj, args, ...others) => {
            ...
            const queryName = models[association.collection]._attributes[association.via].collection ?
            `${models[association.collection]._attributes[association.via].collection}populate` +
            `${models[association.collection]._attributes[association.via].via}` : pluralize(association.collection);

            if (!models[association.collection]._attributes[association.via].collection) {
              parentArgs[association.via] = parentObj[primaryKey];
            }

            return global.queries[queryName].resolve(parentObj, _.merge({}, args, parentArgs), ...others);
          },
        };
      }
    });
...
// TODO: ADD COMMENT
function resolveFindPopulated(model, populate, populateModel, parentObj, { ...args }) {
  const key = {
    model: model.identity,
    populate,
    populateModel: populateModel.identity,
    primaryKey: parentObj[model.primaryKey],
    args,
  };

  return findPopulatedDataLoader.load(JSON.stringify(key));
}
  ...
  _.forEach(types, (type, name) => {
    ...
    _.forEach(model.associations, (association) => {
      if (
        ...
        models[association.collection] &&
        models[association.collection]._attributes[association.via] &&
        models[association.collection]._attributes[association.via].collection &&
        models[association.collection]._attributes[association.via].via
      ) {
        const queryName = `${models[association.collection]._attributes[association.via].collection}populate` +
          `${models[association.collection]._attributes[association.via].via}`;

        builtQueries[queryName] = {
          type: new GraphQLList(type),
          args: _.merge({}, buildFilterArgs(model), utilArgs),
          resolve: (...para) => resolveFindPopulated(models[models[association.collection]._attributes[association.via].collection],
            models[association.collection]._attributes[association.via].via, models[association.collection], ...para),
        };
        global.queries[queryName] = builtQueries[queryName];
      }
    });
  });
...
export default {
  createSchema(models, reqObj) {
    ...
    findPopulatedDataLoader = new DataLoader(keys => new Promise((resolve) => {
      const findPopulatedPromise = key => new Promise((findPopulatedResolve, findPopulatedReject) => {
        const keyObj = helper.tryParseJSON(key);
        const { model, args, populate, populateModel, primaryKey } = keyObj;

        if (!model || !global.models[model]) return findPopulatedReject(new Error(`Invalid model (passed: "${model}").`));
        if (!populateModel || !global.models[populateModel]) {
          return findPopulatedReject(new Error(`Invalid populate model (passed: "${populateModel}").`));
        }

        const Model = global.models[model];
        const PopulateModel = global.models[populateModel];
        const { sort, page, limit, ...criteria } = args;

        return Model
          .find(primaryKey)
          .populate(populate, {
            where: getModelFilters(PopulateModel, criteria),
            skip: (normalizePage(page) - 1) * normalizeLimit(limit),
            limit: normalizeLimit(limit),
            sort: normalizeSort(PopulateModel, sort),
          })
          .then((results) => {
            Model
              .find(primaryKey)
              ...
              .exec((err, resultAfterResults) => {
                if (err) return findPopulatedReject(err);

                const toJsonResult = (results && results[0] && typeof results[0].toJSON === 'function') ? results[0].toJSON() : null;
                const populatedJsonResults = (toJsonResult && toJsonResult[populate]) ? toJsonResult[populate] : null;

                const finalPopulatedJsonResults = [];
                if (populatedJsonResults && populatedJsonResults.length) {
                  _.forEach(populatedJsonResults, (populatedJsonResult) => {
                    finalPopulatedJsonResults.push(_.merge({}, populatedJsonResult, { hasMore: true }));
                  });

                  finalPopulatedJsonResults[finalPopulatedJsonResults.length - 1].hasMore = (
                    resultAfterResults &&
                    resultAfterResults[0] &&
                    resultAfterResults[0][populate] &&
                    resultAfterResults[0][populate].length
                  );
                }

                return findPopulatedResolve(finalPopulatedJsonResults);
              });
          })
          .catch(e => findPopulatedReject(e));
      });

      return resolve(Promise.all(keys.map(key => findPopulatedPromise(key))));
    }));
