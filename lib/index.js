    ...
    configure() {
      ...
      if (sails.hooks.http) {
        ...
        if (_.isUndefined(sails.config.http.middleware.cors)) {
          const corsConfig = sails.config.cors ? _.clone(sails.config.cors) : {};
          if (_.isString(corsConfig.origin) && _.trim(corsConfig.origin) !== '*') {
            corsConfig.origin = corsConfig.origin.split(',');
          }

          sails.config.http.middleware.cors = cors(corsConfig);
        }

        // Add our custom http middleware 'simpleGraphQLMiddleware'
        sails.config.http.middleware.simpleGraphQLMiddleware = (req, res, next) => {
          if (url.parse(req.url).pathname === config.graphqlPath) {
